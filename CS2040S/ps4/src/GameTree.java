...
public class GameTree {
     ...
     */
    public int valueFromNode(TreeNode node, boolean aTurn) {
        if (node == null) {
            ...
            for (TreeNode child : node.children) {
                bestVal = Math.max(valueFromNode(child, false), bestVal);
            }
            node.value = bestVal;
            return node.value;
        } else {
            ...
            for (TreeNode child : node.children) {
                worstVal = Math.min(valueFromNode(child, true), worstVal);
            }
            node.value = worstVal;
            return node.value;
        }
    ...
    int findValue() {
        return valueFromNode(this.root, true);
    }

    public int valueFromNodeMisere(TreeNode node, boolean aTurn) {
        if (node == null) {
            return Integer.MIN_VALUE;
        } else if (node.leaf) {
            return node.value;
            ...
            int bestVal = Integer.MIN_VALUE;
            for (TreeNode child : node.children) {
                bestVal = Math.max(valueFromNodeMisere(child, true), bestVal);
            }
            node.value = bestVal;
            return node.value;
        } else {
            int worstVal = Integer.MAX_VALUE;
            for (TreeNode child : node.children) {
                worstVal = Math.min(valueFromNodeMisere(child, false), worstVal);
            }
            node.value = worstVal;
            return node.value;
        }
    ...
    int findValueMisere() {
        return valueFromNodeMisere(this.root, true);
    }

    public int valueFromNodeNotie(TreeNode node, boolean aTurn) {
        if (node == null) {
            return Integer.MIN_VALUE;
        } else if (node.leaf) {
            ...
            int bestVal = Integer.MIN_VALUE;
            for (TreeNode child : node.children) {
                bestVal = Math.max(valueFromNodeNotie(child, false), bestVal);
            }
            node.value = bestVal;
            return node.value;
        } else {
            int worstVal = Integer.MAX_VALUE;
            for (TreeNode child : node.children) {
                worstVal = Math.min(valueFromNodeNotie(child, true), worstVal);
            }
            node.value = worstVal;
            return node.value;
        }
    ...
    int findValueNotie() {
        return valueFromNodeNotie(this.root, true);
    }

    public int valueFromNodeArb(TreeNode node, boolean aTurn) {
        if (node == null) {
            return Integer.MIN_VALUE;
        } else if (node.leaf) {
            int value = node.value;
            ...
            int bestVal = Integer.MIN_VALUE;
            for (TreeNode child : node.children) {
                bestVal = Math.max(valueFromNodeArb(child, false), bestVal);
            }
            node.value = bestVal;
            return node.value;
        } else {
            int worstVal = Integer.MAX_VALUE;
            for (TreeNode child : node.children) {
                worstVal = Math.min(valueFromNodeArb(child, true), worstVal);
            }
            node.value = worstVal;
            return node.value;
        }
    ...
    int findValueArb() {
        return valueFromNodeArb(this.root, true);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(tree.findValueArb());
    }
