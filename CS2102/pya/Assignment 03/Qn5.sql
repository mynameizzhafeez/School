...
SELECT p.uname, count(*) as num
from petowner p join recommends on p.uname = giver
group by p.uname
having count(*) > 0
union
select cp.uname, count(*) num
from (select uname from customers
	 except
	 select uname from petowner) as cp, recommends r, customers c
where r.receiver = cp.uname
and c.uname = r.giver
group by cp.uname
having count(*) > 0
;
